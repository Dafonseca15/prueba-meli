...
export const ProductBuy: React.FC<BuyAndDeliveryProps> = ({ buy_and_delivery, seller }) => {
  ...
  return (
    <div className="product-buy-container" data-testid="product-buy-container">
      {/**1. Datos de envio */}
      <CustomText as="span" bold={true} color="#00a650" size="md">
        {delivery.title} <span className="product-buy-container__text-bold">a todo el país</span>
      </CustomText>
      ...
      {/**4. Datos vendedor */}
      <CustomText size="xs" marginTop="1.5rem">
        Vendido por <span className="product-buy-container__seller">{seller}</span>
      </CustomText>
      ...
      {benefits?.map((benefit, index) => (
        ...
        >
          <img
            ...
            src={
              benefit.icon.includes('return')
                ? ReturnIcon
                : benefit.icon.includes('shield')
                  ? ShieldIcon
                  : MedalIcon
            }
            alt=""
