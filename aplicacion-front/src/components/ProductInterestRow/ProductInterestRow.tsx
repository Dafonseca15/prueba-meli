...
export const ProductInterestRow: React.FC<ProductInterestRowProps> = ({ title, items }) => {
  return (
    <div className="interest-row-content">
      <span className="interest-row-content__title">{title}</span>
        ...
        {items.map((item, index) => (
          <div key={index}>
            ...
            {index < items.length - 1 && (
              <span className="interest-row-content__separator" data-testid="item-separator">
                {' '}
                -{' '}
              </span>
            )}
